...
const Slide: React.FC<SlideProps> = ({
    ...
    const goIndex = (n: string) => {
        if (outerRef.current) {
            ...
            lis[cidx - 1] && (lis[cidx - 1].style.transform = 'translate3d(-' + scaleW + 'px, 0, 0)')
            lis[cidx + 1] && (lis[cidx + 1].style.transform = 'translate3d(' + scaleW + 'px, 0, 0)')
        }
    }
    // 手指按下的处理事件
    ...
    const moveHandler = (e: any) => {
        ...
        if (target.nodeName === 'IMG') {
            ...
            if (joinPinchScale && e.touches.length >= 2) {
                ...
                // 通过scale设置方法系数
                target.style.webkitTransform = 'scale3d(' + pinchScale + ', ' + pinchScale + ', 1)';
                return
            }
        }
        ...
        //处理双击,双指放大状态时的拖动行为
            ...
            // 首先将动画暂停
            target.style.webkitTransition = 'none';
            target.style.webkitTransform = 'scale3d(' + _scale + ', ' + _scale + ', 1) translate3d(' + (_offsetX * 0.5) + 'px, ' + (_offsetY * 0.5) + 'px, 0)';
            return
        }
    ...
    const dbHandler = (e: any) => {
        ...
        if (target.nodeName === 'IMG') {
            if (joinDbClickScale || joinPinchScale) {
                ...
                target.style.webkitTransition = '-webkit-transform .2s ease-in-out'
                target.style.webkitTransform = 'scale3d(' + scaleMax + ',' + scaleMax + ',1)'
                ...
                <ul className="r-slide-actions">
                    {
                        list!.length >= 2 ?
                            list!.map((item, index) => {
                                return (
                                    <li key={index} className={combineClass('r-slide-actions-dot', idx === index ? 'active' : '')}>•</li>
                                )
                            }) : null
                    }
                </ul>
